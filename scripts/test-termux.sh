...
grep -q '"command": "status"' "$TEST_DIR/mobdesk-status.json"
grep -q '"success": true' "$TEST_DIR/mobdesk-status.json"
grep -q '"state":' "$TEST_DIR/mobdesk-status.json"
grep -q '"message":' "$TEST_DIR/mobdesk-status.json"
grep -q '"workspace"' "$TEST_DIR/mobdesk-status.json"
! grep -q '"ubuntu"' "$TEST_DIR/mobdesk-status.json"
"$MOBDESK_TEST_BIN" version --json > "$TEST_DIR/mobdesk-version.json"
grep -q '"command":"version"' "$TEST_DIR/mobdesk-version.json"
grep -q '"success":true' "$TEST_DIR/mobdesk-version.json"
grep -q '"state":"current"' "$TEST_DIR/mobdesk-version.json"
grep -q '"message":' "$TEST_DIR/mobdesk-version.json"
