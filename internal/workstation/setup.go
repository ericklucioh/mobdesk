...
func (s Service) Setup(ctx context.Context, options SetupOptions) (result SetupResult, err error) {
	...
	if !s.setupPhaseDone("directories") {
		...
		if err := s.ensurePrivateDir(s.Paths.ConfigDir()); err != nil {
			return result, fmt.Errorf("create Mobdesk shell configuration: %w", err)
		}
	...
	if err := s.configureShell(); err != nil {
		return result, err
	}
...
func (s Service) ensureWorkspace() error {
	path := s.Paths.Workspace()
	if info, err := s.Deps.Lstat(path); err == nil && info.Mode()&os.ModeSymlink != 0 {
		return fmt.Errorf("refuse workspace that is a symbolic link: %s", path)
	...
	return s.Deps.Chmod(path, 0o700)
}

func (s Service) configureShell() error {
	if err := s.writePrivateFile(s.Paths.ShellConfig(), []byte(renderShellConfig(version.Value))); err != nil {
		return fmt.Errorf("write shell configuration: %w", err)
	}
	bashrc := filepath.Join(s.Paths.Home, ".bashrc")
	if info, err := s.Deps.Lstat(bashrc); err == nil && info.Mode()&os.ModeSymlink != 0 {
		return fmt.Errorf("refuse shell configuration that is a symbolic link: %s", bashrc)
	} else if err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("check shell configuration: %w", err)
	}
	contents, err := s.Deps.ReadFile(bashrc)
	if err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("read shell configuration: %w", err)
	}
	if strings.Contains(string(contents), "# >>> mobdesk >>>") {
		return nil
	}
	if len(contents) > 0 && contents[len(contents)-1] != '\n' {
		contents = append(contents, '\n')
	}
	contents = append(contents, []byte(shellSourceBlock)...)
	if err := s.Deps.WriteFile(bashrc, contents, 0o600); err != nil {
		return fmt.Errorf("update shell configuration: %w", err)
	}
	return s.Deps.Chmod(bashrc, 0o600)
}

func renderShellConfig(mobdeskVersion string) string {
	return fmt.Sprintf(`# Generated by Mobdesk. Native Termux shell additions.
case ":$PATH:" in
    *:"$HOME/.local/bin":*) ;;
...
if [[ -n "${SSH_TTY:-}" && -n "${SSH_CONNECTION:-}" ]]; then
    mobdesk_version=%s
    mobdesk_configured_port=%d
    IFS=' ' read -r mobdesk_client_host mobdesk_client_port mobdesk_server_host mobdesk_server_port <<< "$SSH_CONNECTION"
    mobdesk_server_host="${mobdesk_server_host:-${HOSTNAME:-unknown}}"
    mobdesk_server_port="${mobdesk_server_port:-$mobdesk_configured_port}"
    mobdesk_user="${USER:-${LOGNAME:-unknown}}"

    printf '\n\033[1;36m╭──────────────────────────────────────────────╮\033[0m\n'
    printf '\033[1;36m│\033[0m  \033[1;32mWelcome to Mobdesk\033[0m                       \033[1;36m│\033[0m\n'
    ...
    mobdesk_goodbye() {
        printf '\n\033[1;35mGoodbye from Mobdesk.\033[0m\n'
    }
    trap mobdesk_goodbye EXIT
fi
`, shellQuote(mobdeskVersion), SSHPort)
}
